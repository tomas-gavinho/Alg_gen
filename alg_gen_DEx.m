function res = alg_gen_DEx(prob,restricciones,bin_real,tam_pob,p_cruza,p_muta,n_var,t_muta,t_sel)
%versión pre_alfa1
...
%     elite = Pob_sel( Metadatos( pos_elite,1 ),:);
    ...
    for t = 1:ctd_ciclos
        ...
        Pob_sel                    = seleccion_DEx (   Pob_sel, Metadatos, bin_real, t_sel);
        Pob_sel                    = cruza_un      (   Pob_sel, bin_real, p_cruza ,rango  );
        Pob_sel                    = muta_DEx      (   Pob_sel, bin_real,t_muta ,p_muta  ,rango , 2  );
        Metadatos(:,2)             = evalua_pob_un (   Pob_sel, restricciones ,bin_real,prob, rango,n_var ); 
